...
if(use_get),function takes values from http get url
//bpl_type, bpl_id, page;
* @return   html content  ( description_of_the_return_value )
*/
function bp_like_show_all_post_likes($atts){
	...
	$all_likers_id = BPLIKE_LIKES::get_likers($item_id, $item_type);
	...
	$bp_like_members_display_query = "include=".$all_likers_id."&page=".$page."&max=".$max."&per_page=".$per_page;

	add_filter('bp_ajax_querystring', "bp_like_pass_members_query_to_core", 999);

	$template_location = apply_filters("bp_like_members_template_location", "members/members-loop", $item_id, $item_type);
	ob_start();
...
add_shortcode( 'bp-likers', 'bp_like_show_all_post_likes' );
